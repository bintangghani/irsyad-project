...
<main class="max-w-screen min-h-screen w-full">
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div class="w-full bg-white px-6 py-4 rounded-xl flex flex-col gap-10">
            <div class="flex justify-between items-center">
                <h1 class="text-4xl font-bold">Permission</h1>
                <button id="tambahPermissionBtn"
                            ...
                            @foreach ($permission as $key => $item)
                                <tr class="bg-white border-b">
                                    ...
                                    <td class="px-6 py-4 flex gap-3 items-center">
                                        <button 
                                            class="editPermissionBtn px-4 py-1 rounded-lg bg-yellow-500 text-white capitalize font-medium hover:bg-yellow-600 cursor-pointer"
                                            data-id="{{ $item->id_permission }}" 
                                            data-nama="{{ $item->nama }}">
                                            Edit
                                        </button>
    ...
    <div id="modalTambahPermission" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 justify-center items-center">
        <form action="{{ route('dashboard.permission.store') }}" method="POST" class="bg-white max-w-lg w-full rounded-xl shadow-md py-6 px-8 flex flex-col gap-6">
            @csrf
            <h2 class="text-2xl font-bold text-center capitalize">Tambah Permission</h2>
            <div>
    ...
    <div id="modalEditPermission" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 justify-center items-center">
        <form action="{{ route('dashboard.permission.update') }}" method="POST" class="bg-white max-w-lg w-full rounded-xl shadow-md py-6 px-8 flex flex-col gap-6">
            @csrf
            @method('PUT')
            <h2 class="text-2xl font-bold text-center capitalize">Edit Permission</h2>
            <input type="hidden" name="id_permission" id="editIdPermission">
            <div>
