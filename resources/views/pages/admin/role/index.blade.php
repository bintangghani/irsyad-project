    ...
    <main class="max-w-screen min-h-screen w-full">
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            <div class="w-full bg-white px-6 py-4 rounded-xl flex flex-col gap-10">
                <div class="flex justify-between items-center">
                    <h1 class="text-4xl font-bold">Role</h1>
                        ...
                        @if ($role->count() > 0)
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 ">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 w-1/2">Nama</th>
                                ...
                                @foreach ($role as $key => $item)
                                    <tr class="bg-white border-b">
                                        ...
                                        <td class="px-6 py-4 capitalize">
                                            ...
                                            <form action="{{ route('dashboard.role.update', ['id' => $item->id]) }}"
                                                method="POST" class="hidden flex items-center gap-3 editJenisInput">
                                                @csrf
                                                @method('PUT')
                                        ...
                                        <td class="px-6 py-4 flex gap-3 items-center">
                                            ...
                                            <form action="{{ route('dashboard.role.destroy') }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <input type="hidden" name="id_role" value="{{ $item->id_role }}">
                                                <button type="submit"
        ...
        <div id="modalTambahJenis"
            ...
            <form action="{{ route('dashboard.role.store') }}" method="POST"
                ...
                @csrf
                <h2 class="text-2xl font-bold text-center capitalize">tambah role</h2>
                <div>
