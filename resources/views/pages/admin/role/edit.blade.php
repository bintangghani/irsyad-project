    ...
    <main class="container-wrapper">
        <div class="container-xxl py-4 px-0">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('dashboard.user.role.update', $role->id_role) }}" method="POST" class="needs-validation" novalidate>
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="nama" id="nama" value="{{ old('nama', $role->nama) }}"
                                required class="form-control @error('nama') is-invalid @enderror">
                            @error('nama')
                                    ...
                                    @foreach ($items as $item)
                                        <div class="form-check mt-3">
                                            <input class="form-check-input" type="checkbox"
                                                value="{{ $item->id_permission }}" id="check{{ $item->id_permission }}"
                                                name="permission[]"
                                                {{ in_array($item->id_permission, $rolePermissions ?? []) ? 'checked' : '' }} />
