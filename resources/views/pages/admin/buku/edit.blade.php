    ...
    <main class="container-wrapper">
        <div class="container-xxl py-4 px-0">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('dashboard.buku.update', $buku->id_buku) }}" method="POST" class="needs-validation"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="alamat_penerbit" id="alamat_penerbit"
                                value="{{ old('alamat_penerbit', $buku->alamat_penerbit) }}" required
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" name="sampul" id="sampul" class="form-control @error('sampul') is-invalid @enderror" accept="image/*">
                            @if ($buku->sampul)
                                <img src="{{ asset('storage/' . $buku->sampul) }}" alt="Profile Picture" class="img-thumbnail mt-2" width="100">
                            @endif
                            @error('sampul')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            ...
                            <select name="sub_kelompok" id="sub_kelompok" required
                                ...
                                @foreach ($subkelompok as $sub)
                                    <option value="{{ $sub->id_sub_kelompok }}"
                                        {{ old('sub_kelompok', $buku->sub_kelompok) == $sub->id_sub_kelompok ? 'selected' : '' }}>
                                        {{ $sub->nama }}</option>
                                @endforeach
                            </select>
                            @error('sub_kelompok')
                            ...
                            <select name="jenis" id="jenis" required
                                ...
                                @foreach ($jenis as $j)
                                    <option value="{{ $j->id_jenis }}"
                                        {{ old('jenis', $buku->jenis) == $j->id_jenis ? 'selected' : '' }}>
                                        {{ $j->nama }}</option>
                                @endforeach
                            </select>
                            @error('jenis')
