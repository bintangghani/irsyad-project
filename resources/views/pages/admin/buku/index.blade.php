            ...
            <div class="card">
                <div class="card-header d-flex flex-column flex-md-row align-items-center justify-content-between">
                    ...
                    <div class="d-flex flex-wrap gap-2 mt-2 mt-md-0">
                        <a href="{{ route('dashboard.buku.import.form') }}" class="btn btn-secondary">
                            <i class="bx bx-upload me-2"></i>
                            <span class="d-none d-sm-inline-block">Import Buku</span>
                        </a>
                            ...
                            <form action="{{ route('dashboard.buku.index') }}" method="GET" id="paginationForm">
                                <select class="form-select" name="per_page"
                                    ...
                                    @foreach ([10, 25, 50, 75, 100] as $value)
                                        <option value="{{ $value }}"
                                            {{ request('per_page') == $value ? 'selected' : '' }}>{{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                            </form>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center text-truncate">
                                                @if ($item->file_buku)
                                                    <a href="{{ asset('storage/' . $item->file_buku) }}" target="_blank"
                                                        class="btn btn-primary btn-sm">Buka Buku</a>
                                                @else
                                            ...
                                            <td class="text-center align-middle">
                                                <div class="d-flex flex-column gap-1 align-items-center">
                                                    ...
                                                    <form action="{{ route('dashboard.buku.destroy', $item->id_buku) }}"
                                                        method="POST" data-confirm-delete="true" class="w-100">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
