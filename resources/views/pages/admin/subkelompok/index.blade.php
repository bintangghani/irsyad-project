...
@section('title', 'User - subkelompok')
...
@section('content')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item active">subkelompok</li>
        </ol>
    </nav>
        ...
        <div class="container-xxl py-4 px-0">
            <div class="card">
                <div class="card-header d-flex flex-column flex-md-row align-items-center justify-content-between">
                    <h5 class="card-title mb-0 fs-3">List subkelompok</h5>
                    <a href="{{ route('dashboard.subkelompok.create') }}" class="btn btn-primary">
                        <i class="bx bx-plus me-2"></i>
                        <span class="d-none d-sm-inline-block">Tambah subkelompok Baru</span>
                    </a>
                </div>
                            ...
                            @if ($subkelompok->count() > 0)
                                <thead class="table-light">
                                    <tr class="bg-primary">
                                        ...
                                        <th scope="col" class="bg-primary text-white max-w-[100px] w-30">Sub Kelompok</th>
                                    ...
                                    @foreach ($subkelompok as $key => $item)
                                        <tr>
                                            ...
                                            <td class="text-capitalize">
                                                <div class="editsubkelompokSection">{{ $item->nama }}</div>
                                            </td>
                                            <td class="text-capitalize">
                                                <div class="editsubkelompokSection">{{ $item->kelompok->nama }}</div>
                                            </td>
                                            <td class="text-center">
                                                <button class="btn btn-warning editsubkelompokBtn">Edit</button>
                                                <form action="{{ route('dashboard.subkelompok.destroy') }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <input type="hidden" name="id_subkelompok" value="{{ $item->id_subkelompok }}">
