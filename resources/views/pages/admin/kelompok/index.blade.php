...
@section('content')
    <main class="max-w-screen min-h-screen w-full">
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            <div class="w-full bg-white px-6 py-4 rounded-xl flex flex-col gap-10">
                <div class="flex justify-between items-center">
                    <h1 class="text-4xl font-bold">Kelompok</h1>
                                ...
                                @foreach ($kelompok as $key => $item)
                                    <tr class="bg-white border-b">
                                        ...
                                        <td class="px-6 py-4 capitalize">
                                            ...
                                            <form action="{{ route('dashboard.kelompok.update', ['id' => $item->id]) }}" method="POST"
                                                class="hidden flex items-center gap-3 editJenisInput">
                                                @csrf
                                                @method('PUT')
                                                <input type="hidden" name="id_jenis" value="{{ $item->id_kelompok }}">
        ...
        <div id="modalTambahJenis"
            ...
            <form action="{{ route('dashboard.kelompok.store') }}" method="POST"
